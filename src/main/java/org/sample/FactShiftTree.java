...
public class FactShiftTree {
    ...
    public static BigInteger factShift(int n) {
		...
		// Для наглядности посчитаем, сколько раз двойка содержится в 10! Двойку дает каждый второй множитель
		// (2, 4, 6, 8 и 10),всего таких множителей 10 / 2 = 5. Каждый четвертый дает четверку (2^2), всего таких
		// множителей 10 / 4 = 2 (4 и 8). Каждый восьмой дает восьмерку (2^3), такой множитель всего один 10 / 8 = 1 (8).
		// Шестнадцать (2^4) и более уже не дает ни один множитель, значит, подсчет можно завершать. Суммируя, получим,
        // что показатель степени при двойке в разложении 10! на простые множители будет равен
        ...
        // (e.g. 1 * 2 * 3 * 4 * 5 * 6. One two in 2, and two twos in 4 and one 2 in 6. So [c] = 4.)
        int p = 0, c = 0;
        while((n >> p) > 1) {
            p++;
            c += n >> p;
